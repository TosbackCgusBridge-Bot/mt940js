...
class BankDate {
  ...
  static parse(year, month, day) {
    ...
    let d = new Date(Date.UTC(fullyear, Number.parseInt(month, 10) - 1, day));
    if ((d.getMonth() + 1) !== parseInt(month) || d.getFullYear() !== fullyear) {
      throw Error ( `Date in the bank payment file is invalid: ${year + '' + month + '' + day}` );
    }
    return d;
  }
}
